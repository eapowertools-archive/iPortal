...
var qrsInteract = 
{
	...
	post: function(path,body)
	{
		return new Promise(function(resolve, reject)
		{
			...
			.on('response', function(response, body)
			{
				logger.debug('post::response code::' + response.statusCode, {module: 'qrsinteraction'});
				sCode = response.statusCode;
			})
			...
			.on('data', function(data)
			{
				...
				{
					logger.debug('post::Response from QRS::' + JSON.stringify(JSON.parse(data)), {module: 'qrsinteraction'});
					resolve(data);
				}
				else if(sCode==204)
				{
					logger.debug('post:Response from QRS::204 No Content', {module:'qrsinteraction'});
					resolve(sCode);
				}
				else
				{
